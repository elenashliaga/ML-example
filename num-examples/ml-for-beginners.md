...
Может показаться, что машинное обучение основано на сложном наборе правил, но, на самом деле, оно построено на простых и понятных принципах. Цель этой статьи -- объяснить эти принципы.

Чтобы раскрыть принципы:
* я разберу [схему обучения моделей](#что-такое-supervised-learning);
* на двух простых примерах моделей для обучения разберу [немного математики, которая за всем стоит](#математические-основы);
* оставялю в последнем разделе [примеры кода для реализации разобранных моделей для обучения](#реализация-в-коде).
...
Такой тип обучения я буду рассматривать в задачах классификации.
...
Для вас скорее всего это уже будет подозрительно. Наивный байесовский классификатор же во время обучения выделит из текста письма слова, который указывают на спам, и будет их исполльзовать для определения.

> [!NOTE]  
> Классификатор наивный, потому что предполагает, что критерии спама независимы друг от друга.

Для определения спама Наивный байесовский классификатор построит два графика:
* график вероятности встретить слово для первого класса: письмо --спам.
* график вероятности встретить слово для второго класса: письмо не спам.
