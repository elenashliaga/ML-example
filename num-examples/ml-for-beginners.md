...
На этапе обучения модель с помощью математики составляет правило, по которому она будет распределять на группы. Этот этап для большинства людей, да и для меня тоже, является некоторой магией. Поэтому я разберу его на двух простых математических моделях: линейный дискриминантный анализ (LDA) и наивный байесовский классификатор.
